		...
		function Sidebar() {
			...
			this.init = function() {
				...
				if (self.o.search) {
					...
					var input = $('<input>').attr({'type': 'text', 'spellcheck': 'false', 'placeholder': '搜索...'}).addClass('mapplic-search-input').keyup(function() {
				...
				this.notfound = $('<p></p>').addClass('mapplic-not-found').text('什么也没有找到~~请换一个搜索对象试试……').appendTo(listContainer);
